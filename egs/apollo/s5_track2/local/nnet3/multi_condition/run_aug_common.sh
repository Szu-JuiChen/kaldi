...
if [ $stage -le 4 ]; then
  ...
  for dataset in ${train_set}_aug; do
    ...
    utils/fix_data_dir.sh data/${dataset}_hires || exit1;
  done
fi
...
if [ $stage -le 5 ]; then
  for dataset in $test_sets; do
    ...
    utils/copy_data_dir.sh data/$dataset data/${dataset}_hires
    steps/make_mfcc.sh --cmd "$train_cmd" --nj 20 --mfcc-config conf/mfcc_hires.conf \
        data/${dataset}_hires || exit 1;
    steps/compute_cmvn_stats.sh data/${dataset}_hires || exit 1;
    utils/fix_data_dir.sh data/${dataset}_hires || exit 1; # remove segments with problems
  done
fi
...
if [ "$use_ivectors" == "true" ]; then
  if [ $stage -le 6 ]; then
    ...
    utils/filter_scp.pl -f 2 data/${train_set}/utt2spk data/${train_set}_aug_hires/utt2uniq | \
    ...
    steps/compute_cmvn_stats.sh data/${ivector_trainset}_aug_hires
    utils/fix_data_dir.sh data/${ivector_trainset}_aug_hires
  fi
  ...
  if [ $stage -le 7 ]; then
    ...
    steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
      ...
      data/lang exp/${lda_mllt_ali}_aug exp/nnet3${nnet3_affix}/tri3b
  fi
  ...
  if [ $stage -le 8 ]; then
    ...
    steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 200000 \
      data/${train_set}_aug_30k_nodup_hires 512 exp/nnet3${nnet3_affix}/tri3b exp/nnet3${nnet3_affix}/diag_ubm
  fi
  ...
  if [ $stage -le 9 ]; then
    ...
    steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
      data/${ivector_trainset}_aug_hires exp/nnet3${nnet3_affix}/diag_ubm exp/nnet3${nnet3_affix}/extractor || exit 1;
  fi
  ...
  if [ $stage -le 10 ]; then
    ...
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
      data/${train_set}_aug_max2_hires exp/nnet3${nnet3_affix}/extractor exp/nnet3${nnet3_affix}/ivectors_${train_set}_aug || exit 1;
